...
async function handleIndexRequest(request, env) {
    ...
    try {
        const listResult = await bucket.list({ delimiter: '/' });
        if (!listResult.delimitedPrefixes.length) {
            return new Response('Server configuration error: No tag directories found in bucket.', { status: 500 });
        }
        const tagName = listResult.delimitedPrefixes[0].slice(0, -1);

        // 并行获取所有 manifest 文件来计算数量
        const manifestPromises = AVAILABLE_ORIENTATIONS.map(orient =>
            bucket.get(`${tagName}/${orient}/manifest.json`)
        );
...
async function handleApiRequest(request, env) {
    ...
    try {
        ...
        if (orientation === 'any') {
            ...
            const manifestPromises = AVAILABLE_ORIENTATIONS.map(orient =>
                bucket.get(`${tagName}/${orient}/manifest.json`).then(obj => obj ? obj.json() : [])
            );
            ...
            const manifestObject = await bucket.get(`${tagName}/${orientation}/manifest.json`);
        ...
        const imageKey = `${tagName}/${finalImageOrientation}/${finalImageName}`;
